function DCM = ncl_spm_dcm_csd_data(DCM)
...
% Frequencies
%--------------------------------------------------------------------------
% DCM.xY.Hz  = fix(Hz1:Hz2);             % Frequencie                      <- allow for different frequency resolution
...
for i = 1:Ne;
    ...
    for k = 1:Nw
        ...
        for j = 1:Nt
            ...
            covscale    = 0; 
            try 
                mar.noise_cov = DCM.xY.mar{i}.noise_cov;                   % <- use existing noise covariance for csd scaling
                covscale      = 1;
            end
            mar = ncl_spm_mar_spectra(mar,DCM.xY.Hz,1/DCM.xY.dt);           % <- 
            P   = P + mar.P;
        end
...
if covscale,    disp('Used pre-specified noise covariance');    end